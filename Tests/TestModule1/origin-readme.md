...
#### КРАТКОЕ ОПИСАНИЕ [Get-AboutTest][]
...
по данным модуля и комментариям к его функциям.
	...
	Get-AboutTestFunction [-ModuleInfo] <PSModuleInfo> [-UICulture <CultureInfo>] [-ReferencedModules <PSModuleInfo[]>]
...
Для выполнения функции `Get-AboutTest` требуется роль **Everyone** для учётной записи,
...
[Get-AboutTest]: <../it-service/itg.readme#get-aboutmodule> "Генерирует содержимое файла `about_$(ModuleInfo.Name).txt` с MarkDown разметкой по данным модуля и комментариям к его функциям."
